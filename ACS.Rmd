...
```{r chunk3, eval = TRUE, echo = TRUE, results = "show", warning = FALSE, error = FALSE, message = FALSE, fig.width = 12}
library("ITHIM")
msaIDFile <- read.csv(system.file("join2.csv", package = "ITHIM"), stringsAsFactors = FALSE)
msaIDFile %>% filter(msaNHTS  == 6442) %>% select(stateFIPS,countyFIPS)
```